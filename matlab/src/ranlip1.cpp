...
int	CRanLip::SavePartition(char * fname)
{
...
int	CRanLip::LoadPartition(char * fname)
{
	int i,j;	
	FreeMem();
	...
	for(j=0;j<Dimension;j++) {
		if(fscanf(fp,"%lf %lf\n",&t_boundLeft[j], &t_boundRight[j])!=2) {fclose(fp); return 1;};
	}
	...
	if(Probabilities==NULL) return 2;

	for(j=0;j<TotalElements;j++) {
		if(fscanf(fp,"%lf\n",&r)!=1) {fclose(fp); return 1;};
		Probabilities[j] = r;
	}
	fclose(fp);
